...
int APIENTRY wWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, PWSTR pCmdLine, int nCmdShow){
	...
	/*카운트에따라 모니터 좌표 찾아서 스크린락확장
	int test=GetSystemMetrics(SM_CMONITORS);
	if(test==0)MessageBox(0,TEXT("0"),TEXT("0"),MB_OK);
	if(test==1)MessageBox(0,TEXT("1"),TEXT("1"),MB_OK);
	if(test==2)MessageBox(0,TEXT("2"),TEXT("2"),MB_OK);
	*/
	HHOOK hookk,hookm;
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT iMessage, WPARAM wParam, LPARAM lParam){
	...
	case WM_CREATE:
		...
		CreateWindow(TEXT("button"),TEXT("UNLOCK"),WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON, width/2-50,height/2+25,100,25,hWnd,(HMENU)ID_BUTTON,g_hInst,NULL);
		return 0;
	case WM_COMMAND:
		wchar_t str[128];
...
LPVOID Register(){
	...
	if(lResult != ERROR_SUCCESS){
		DWORD dwDesc;
		lResult = RegCreateKeyEx(HKEY_LOCAL_MACHINE, TEXT("SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Run"), 0, buffer, REG_OPTION_NON_VOLATILE, KEY_ALL_ACCESS, NULL, &hKey, &dwDesc);
		if(lResult == ERROR_SUCCESS) {
